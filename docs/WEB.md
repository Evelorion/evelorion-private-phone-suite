...
| 通话记录 | 只读。用的是 `HKDF(DEK,"fc.collection.calls.v1")` 派生的另一把密钥 |
...
| 安全 | 改自己的口令、看管理员会话、看最近的登录失败（发现爆破） |
...
改完直接重建镜像即可，见 [DEVELOPMENT.md](DEVELOPMENT.md)。
