...
权限流程遵守 Google Play 的短信权限规则：必须先由用户在系统界面授予默认短信
角色，应用之后才请求 `READ_SMS`、`SEND_SMS`、`RECEIVE_SMS` 等权限；失去默认
角色后立即停止读取和发送系统短信。Release 同时生成 APK 与供 Play Console
提交的 AAB，目标版本为 Android 16（API 36）。
